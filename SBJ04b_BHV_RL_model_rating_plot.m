...
ez_rat = scatter(bhv.tol(ez_trl_idx), bhv.rating(ez_trl_idx), trl_rat_sz,'r','*');
ez_blk = scatter(blk_tol(blk_ez),blk_acc(blk_ez), mean_sz,'k','filled');
ez_rat_blk = scatter(blk_tol(blk_ez),blk_rat(blk_ez), mean_rat_sz,'r','filled','MarkerEdgeColor','k');

% Plot Trial and Block Behavior: Hard
hd_trl = scatter(bhv.tol(~ez_trl_idx), bhv.hit(~ez_trl_idx), trl_sz,'k','d');
hd_rat = scatter(bhv.tol(~ez_trl_idx), bhv.rating(~ez_trl_idx), trl_rat_sz,'r','+');
hd_blk = scatter(blk_tol(~blk_ez),blk_acc(~blk_ez), mean_sz,'k','d','filled');
hd_rat_blk = scatter(blk_tol(~blk_ez),blk_rat(~blk_ez), mean_rat_sz,'r','d','filled','MarkerEdgeColor','k');
