    ...
    for s = 1:numel(SBJs)
        ...
        if plot_peaks && (any(miss_erps(s,cond_ix)) || bad_pks(s,cond_ix))
            ...
            plot(time_vec(ft_rng(1,1):ft_rng(2,2)),...
                squeeze(erps(cond_ix,s,ft_rng(1,1):ft_rng(2,2))),...
...
if plot_feat
    ...
    for cond_ix = 1:numel(cond_lab)
        ...
        for s = 1:numel(SBJs)
            ...
            if ~miss_erps(s,cond_ix)
                % Plot Peaks
                scatter(pk_times(s,cond_ix,:),pk_amp(s,cond_ix,:), 'o', ...
...
fprintf('Saving %s\n',stat_out_fname);
save(stat_out_fname,'-v7.3','SBJs','erp_amp','pk_amp','pk_times','miss_erps','bad_pks');
