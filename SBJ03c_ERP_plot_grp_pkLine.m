...
for ch_ix = 1:numel(ch_list)
    ...
    % Find peak limit indices
    pk_idx = [0 0];
    [~, pk_idx(1)] = min(abs(time_vec-pk_lim(1)));
    [~, pk_idx(2)] = min(abs(time_vec-pk_lim(2)));
    pk_time_vec = time_vec(pk_idx(1):pk_idx(2));
    
    % Compute means and variance
    plt_means = NaN([numel(cond_lab) numel(time_vec)]);
    sems      = NaN([numel(cond_lab) numel(time_vec)]);
    pk_times  = NaN(size(cond_lab));
    for cond_ix = 1:numel(cond_lab)
        plt_means(cond_ix,:) = squeeze(mean(means(cond_ix,:,ch_ix,:),2));
        sems(cond_ix,:) = squeeze(std(means(cond_ix,:,ch_ix,:),[],2))./sqrt(numel(SBJs))';
        % Find peak
        [~, pk_ix] = findpeaks(pk_sign*plt_means(cond_ix,pk_idx(1):pk_idx(2)));
        pk_times(cond_ix) = pk_time_vec(pk_ix);
    end
...
%     subplot(plot_rc(1),plot_rc(2),ch_ix);
    ...
    % Plot Peak Markers
    for cond_ix = 1:numel(cond_lab)
        line([pk_times(cond_ix) pk_times(cond_ix)],ylim,...
            'Color',cond_colors{cond_ix},'LineWidth',plt.evnt_width);
    end
%     ylims = ylim;
%     patch([pk_lim(1) pk_lim(2) pk_lim(2) pk_lim(1)],[ylims(2) ylims(2) ylims(1) ylims(1)],...
%         [0.5 0.5 0.5],'FaceAlpha',plt.errbar_alpha);
