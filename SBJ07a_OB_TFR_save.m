...
load([SBJ_vars.dirs.events SBJ '_behav_' ob_proc_id '_final.mat']);

%% Load ICA, reconstruct and clean
if numel(SBJ_vars.block_name)>2; error('not ready for 3+ blocks!'); end
clean_data = fn_load_clean_experiment_OB(SBJ, ob_proc_id, tt_proc_id);
...
for b_ix = 1:numel(SBJ_vars.block_name)
    % Check that this block hasn't passed the oddball task
    if numel_prev_block < 400
        ...
        if strcmp(an.event_type,'S')                                    % feedback code
            cfg.trialdef.eventvalue = proc.event_code;
        else
            error(['Why use oddball an.event_type: ' an.event_type '?']);
        end
        ...
        cfg.trialfun            = 'oddball_trialfun';                        % custom trial segmentation function
        if b_ix > 1
...
[bhv_orig] = fn_load_behav_csv_oddball([SBJ_vars.dirs.events SBJ '_behav_oddball.csv']);
load([SBJ_vars.dirs.events SBJ '_' ob_proc_id '_02a_orig_exclude_trial_ix.mat']);
...
cfgs = [];
cfgs.trials  = setdiff([1:numel(trials.trial)], SBJ_vars.trial_reject_ix_oddball);
clean_trials = ft_selectdata(cfgs, trials);

for f_ix = 1:numel(bhv_fields)
    bhv_orig.(bhv_fields{f_ix})(SBJ_vars.trial_reject_ix_oddball) = [];
end
...
%% Save Results
data_out_fname = [SBJ_vars.dirs.proc SBJ '_' ob_proc_id '_' an_id '.mat'];
fprintf('Saving %s\n',data_out_fname);
