...
fig_name  = ['model_prediction_comparison' bin_str];
fig_ftype = 'png';
...
if plot_binary
    subplot(2,n_y_subplots,1); hold on;
    title(['Categorical Reward Value Model' cond2_str]);
    set(gca,'FontSize',16);
    ...
    legend([tmp1 tmp2 tmp3], {'Reward Valence','Reward Magnitude','Outcome Likelihood'}, 'Location','southwest');
    ...
    subplot(2,n_y_subplots,2); hold on;
    title(['Reward Prediction Error (RPE) Model' cond3_str]);
    set(gca,'FontSize',16);
    set(gca,'XTick',cond_idx_nn);
    set(gca,'XTickLabels',cond_lab(cond_idx_nn));
    xtickangle(tick_angle);
    ...
    legend([tmp1 tmp2 tmp3], {'RPE','RPE Magnitude','Outcome Likelihood'}, 'Location','southeast');
    [~] = fn_min_white_space(gca);
end

%% ======================= All Outcomes =======================
%--------------------- All Outcomes: Directional Coding ---------------------
subplot(2,n_y_subplots,out_mod_n); hold on;
title(['Reward Value Model' cond2_str]);
set(gca,'FontSize',16);
...
legend([tmp1 tmp2 tmp3], {'Reward Valence','Reward Magnitude','Outcome Likelihood'}, 'Location','southeast');
% legend([tmp1 tmp2 tmp3 tmp4], {'Valence','Magnitude','Likelihood', 'Interaction'}, 'Location','southeast');
if plot_binary; [~] = fn_min_white_space(gca); end

%--------------------- All Outcomes: Prediction Errors ---------------------
subplot(2,n_y_subplots,rpe_mod_n); hold on;
title(['Reward Prediction Error (RPE) Model' cond3_str]);
set(gca,'FontSize',16);
set(gca,'XTick',cond_idx);
set(gca,'XTickLabels',cond_lab);
