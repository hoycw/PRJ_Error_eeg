function SBJ03c_ERP_plot_grp_diffwave_butterfly(SBJs,conditions,proc_id,an_id,plt_id,save_fig,varargin)
...
for s = 1:length(SBJs)
    ...
    means = NaN([numel(cond_lab) numel(ch_list) numel(time_vec)]);
    ...
    for diff_ix = 1:numel(diff_lab)
        if iscell(diff_pairs{diff_ix})
            ...
            diff_means(diff_ix,s,:,:) = means(cond_ix,:,:)-contrast_mean;
        elseif any(diff_pairs{diff_ix}==0)
            cond_ix = diff_pairs{diff_ix}(diff_pairs{diff_ix}~=0);
            diff_means(diff_ix,s,:,:) = means(cond_ix,:,:);
        else
            diff_means(diff_ix,s,:,:) = means(diff_pairs{diff_ix}(1),:,:)-means(diff_pairs{diff_ix}(2),:,:);
        end
    end
    
    clear tmp SBJ_vars bhv roi trials means cond_idx
end
...
for ch_ix = 1:numel(ch_list)
    ...
    %% Create plot
    fig_name = ['GRP_' conditions '_dw_' an_id '_' ch_list{ch_ix} '_but'];    
...
%     subplot(plot_rc(1),plot_rc(2),ch_ix);
    axes = gobjects([numel(diff_lab)+1 1]);
    subplot(numel(diff_lab)+1,1,1);
    axes(1) = gca; hold on;
    ...
    for diff_ix = 1:numel(diff_lab)
        subplot(numel(diff_lab)+1,1,diff_ix+1);
        axes(diff_ix+1) = gca; hold on;
        
        % Plot SBJ Means
        plot(time_vec, squeeze(diff_means(diff_ix,:,ch_ix,:)),'Color',diff_colors{diff_ix},...
            'LineWidth',plt.butterfly_width);
        
        % Plots GRP Mean
        plot(time_vec, plt_means(diff_ix,:), 'Color', 'k', 'LineWidth', plt.mean_width);
        ...
        % Axes and Labels
        axes(diff_ix+1).YLabel.String = 'uV';
        axes(diff_ix+1).XLim          = [plt.plt_lim(1) plt.plt_lim(2)];
