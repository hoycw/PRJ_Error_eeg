function [data] = ft_redefinetrial_allowoverlap(cfg, data)
...
elseif ~isempty(cfg.trl)
  ...
  for iTrl=1:size(trl, 1)
    ...
    data.trial{iTrl} = ft_fetch_data_allowoverlap(dataold, 'header', hdr, 'begsample', begsample(iTrl), 'endsample', endsample(iTrl), 'chanindx', 1:hdr.nChans, 'skipcheckdata', 1);
