function violins = violinplot_mod(data, cats, text_labels, status, colors, varargin)
...
% This code is released under the terms of the BSD 3-clause license
    ...
    text_labels = strrep(text_labels,'EEG','')
    vals_prev = zeros(1, 7)
    for ix = 1:6
         for ix2 = 1:length(text_labels)
             if status(ix2)~=0 & status(ix2)~=3 & status(ix2)~=4 || violins(ix).ScatterPlot.YData(ix2) > 2
                vals_prev(1, violins(ix).ScatterPlot.YData(ix2)+1) = vals_prev(1, violins(ix).ScatterPlot.YData(ix2)+1) + 1
                if mod(vals_prev(1,violins(ix).ScatterPlot.YData(ix2)+1),2) == 1 || violins(ix).ScatterPlot.YData(ix2) == 0
                    text(violins(ix).ScatterPlot.XData(ix2)+0.01, violins(ix).ScatterPlot.YData(ix2)+0.03, text_labels(ix2), 'rotation', 45, 'FontSize', 10, 'Color', colors(ix2,:))
                elseif mod(vals_prev(1,violins(ix).ScatterPlot.YData(ix2)+1),2) == 0 
                    text(violins(ix).ScatterPlot.XData(ix2)-0.04, violins(ix).ScatterPlot.YData(ix2)-0.07, text_labels(ix2), 'rotation', 45, 'FontSize', 10, 'Color', colors(ix2,:), 'VerticalAlignment', 'top')
                end
             end
         end
    end
    title('Absolute Value of the Difference from the Expected Values Across Conditions')
end
