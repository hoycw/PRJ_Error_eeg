function clean_data = fn_load_clean_experiment_OB(SBJ, ob_proc_id, tt_proc_id)
%% Load, clean, and reconstruct full oddball experiment time series
%   Essentially reproduces SBJ02b but without trial segmentation
% INPUTS:
%   SBJ [str] - ID of subject
%   ob_proc_id [str] - ID of oddball task preprocessing pipeline
%   tt_proc_id [str] - ID of target time task preprocessing pipeline
% OUTPUTS:
...
% Reload Data
load([SBJ_vars.dirs.preproc SBJ '_preproc_' tt_proc_id '.mat'],'data','icaunmixing','icatopolabel');
load([SBJ_vars.dirs.preproc SBJ '_' ob_proc_id '_02a.mat'], 'heog_ics','veog_ics');
