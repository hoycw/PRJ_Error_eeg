function [cfg] = ft_databrowser_allowoverlap(cfg, data)
...
if hasdata
  ...
  if ~isempty(cfg.trl)
    ...
    data = ft_redefinetrial_allowoverlap(tmpcfg, data);
...
if isempty(cmenulab)
  ...
  switch cfg.seldat
    case 'current'
      ...
      seldata.trial{1}    = ft_fetch_data_allowoverlap(opt.curdata, 'begsample', begsel, 'endsample', endsel);
    case 'all'
      seldata             = keepfields(opt.org, {'label', 'grad', 'elec', 'opto', 'hdr'});
      seldata.trial{1}    = ft_fetch_data_allowoverlap(opt.orgdata, 'begsample', begsel, 'endsample', endsel);
  end
...
if isempty(opt.orgdata)
  ...
  dat = ft_fetch_data_allowoverlap(opt.orgdata, 'header', opt.hdr, 'begsample', begsample, 'endsample', endsample, 'chanindx', chanindx, 'allowoverlap', istrue(cfg.allowoverlap));
end
art = ft_fetch_data_allowoverlap(opt.artdata, 'begsample', begsample, 'endsample', endsample);
