...
[grp_lab, grp_colors, ~]  = fn_group_label_styles('DifOutDO');
grp_lab{strcmp(grp_lab,'Dif')} = 'pWin';
grp_lab{strcmp(grp_lab,'Out')} = 'sPE';
grp_lab{strcmp(grp_lab,'Dif*Out')} = 'uPE';
...
for grp_ix = 1:numel(grp_lab)
    for t_ix = 1:numel(lme)
        pvals(grp_ix,t_ix) = lme{t_ix}.Coefficients.pValue(grp_ix+1);
    end
end
