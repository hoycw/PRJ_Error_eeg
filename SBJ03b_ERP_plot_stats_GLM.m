...
% Load data
load([SBJ_vars.dirs.proc SBJ '_' stat_id '_' an_id '.mat']);

% Select conditions (and trials)
[grp_lab, grp_colors, ~] = fn_group_label_styles(st.model_lab);
for grp_ix = 1:numel(grp_lab)
    if ~any(strcmp(grp_lab{grp_ix},st.regressors))
        error(['grp ' grp_lab{grp_ix} ' not found in regressors!']);
    end
end
...
sig_ch = zeros([numel(beta.label) numel(grp_lab)]);
for ch_ix = 1:numel(beta.label)
    ...
    for grp_ix = 1:numel(grp_lab)
        regr_ix = strcmp(grp_lab{grp_ix},st.regressors);
        if any(beta.qval(regr_ix,ch_ix,:) <= st.alpha)
            sig_grp(grp_ix) = true;
            sig_ch(ch_ix,grp_ix) = 1;
            sig_chunks{grp_ix} = fn_find_chunks(squeeze(beta.qval(regr_ix,ch_ix,:))<=st.alpha);
            sig_chunks{grp_ix}(squeeze(beta.qval(regr_ix,ch_ix,sig_chunks{grp_ix}(:,1))>st.alpha),:) = [];
        end
    end
    
    %% Create plot
    fig_name = [SBJ '_' stat_id '_' an_id '_' beta.label{ch_ix}];    
    figure('Name',fig_name,'units','normalized',...
        'outerposition',[0 0 0.5 0.5],'Visible',fig_vis);   %this size is for single plots
%     [plot_rc,~] = fn_num_subplots(numel(beta.label));
...
%     subplot(plot_rc(1),plot_rc(2),ch_ix);
    ...
    for grp_ix = 1:numel(grp_lab)
        for sig_ix = 1:size(sig_chunks{grp_ix},1)
            if strcmp(plt.sig_type,'line')
                sig_times = beta.time(sig_chunks{grp_ix}(sig_ix,1):sig_chunks{grp_ix}(sig_ix,2));
            ...
            elseif strcmp(plt.sig_type,'patch')
                ...
                patch(beta.time([sig_chunks{grp_ix}(sig_ix,1) sig_chunks{grp_ix}(sig_ix,1) ...
...
%     ax.YLim          = ylims; %!!! change for plt.sigType=line
    ...
    ax.Title.String  = beta.label{ch_ix};
    if plt.legend
