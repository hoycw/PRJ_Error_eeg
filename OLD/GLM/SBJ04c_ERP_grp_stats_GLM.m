function SBJ04c_ERP_grp_stats_GLM(SBJ_id,proc_id,an_id,stat_id,varargin)
% Compute GLM on SBJ average data and tests betas vs. 0
%   Should only be used for peak-to-peak FRN, otherwise use LME
...
eval(stat_vars_cmd);
if ~strcmp(st.measure,'p2p') || ~strcmp(st.an_style,'glm'); error('GLM should only be run on p2p!'); end
...
elseif any(strcmp(st.measure,{'mean','p2p'}))
    ...
    for ch_ix = 1:numel(ch_list)
        ...
        glm{ch_ix} = fitglm(tbl,formula);
        pvals(:,ch_ix) = glm{ch_ix}.Coefficients.pValue(2:end);
...
fprintf('Saving %s\n',stat_out_fname);
save(stat_out_fname,'-v7.3','glm','qvals','SBJs','time_vec','ch_list','pk_amp','pk_times','data');
